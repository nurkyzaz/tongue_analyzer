...
def main():
    ...
    tr = MultiTaskDataset(args.manifest, args.data_root, "train", args.img_size)
    va = MultiTaskDataset(args.manifest, args.data_root, "val", args.img_size)
    ...
    for ep in range(args.epochs):
        ...
        for step, (img, mask, y, w) in enumerate(tl):
            if args.max_steps and step >= args.max_steps:
                break
            img, mask, y, w = img.to(device), mask.to(device), y.to(device), w.to(device)
            opt.zero_grad()
            with torch.cuda.amp.autocast():
                out = model(img, mask)
                loss = sum(losses[ch](out[ch], y[:, k], w[:, k]) for k, ch in enumerate(KEY_CHARS))
